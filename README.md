# 🖨️↻ PrintSpoolerCleaner v1.0

**PrintSpoolerCleaner** is a lightweight C# tool for quickly fixing printing issues.  
It stops the Print Spooler service, clears stuck print jobs, and restarts the service automatically.

---

## ⚡ Features

- 🛑 **Temporarily stop the Print Spooler**  
- 🗑️ **Clear all stuck print jobs** in the spool folder  
- 🔄 **Restart the service** automatically  
- 🏃 **Fast and easy to use**  
- 🔐 Requires **Administrator privileges**

---

## 💻 Requirements

- Windows 7 / 8 / 10 / 11  
- .NET Framework **4.8**  
- Run the program as **Administrator**

---

## 📦 How to Use

1. Run the program as Administrator  
3. Wait for the "Done" message when the process completes

